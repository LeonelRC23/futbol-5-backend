...
const createRental = async (req, res) => {
    try {
        ...
        `;
        const [conflicts] = await pool.query(checkOverlapQuery, [id_field, rental_date, rental_end, rental_start]);

        if (conflicts.length > 0) {
            return res.status(409).json({ message: "La cancha ya se encuentra reservada en ese horario." });
        }
...
const updateRental = async (req, res) => {
    try {
        ...
        const checkOverlapQuery = `
            ...
            AND id_rental != ?
        `;
        const [conflicts] = await pool.query(checkOverlapQuery, [id_field, rental_date, rental_end, rental_start, id]);

        if (conflicts.length > 0) {
            return res.status(409).json({ message: "El nuevo horario entra en conflicto con una reserva existente." });
        }
        ...
        const updateQuery = `
            ...
            WHERE id_rental = ?
        `;
        const [result] = await pool.query(updateQuery, [id_field, rental_price, id_user, rental_date, rental_start, rental_end, id]);
...
const deleteRental = async (req, res) => {
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: "Reserva no encontrada o ya fue eliminada." });
        }
